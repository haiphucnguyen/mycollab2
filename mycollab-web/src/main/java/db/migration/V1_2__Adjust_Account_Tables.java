		...
		for (Record user : users) {
			...
			jdbcTemplate
					.update("insert into s_user_account (username, accountId, isAdmin, isAccountOwner, roleId, registeredTime, registerStatus, lastAccessedTime, registrationSource ) values (?,?,?,?,?,?,?,?,?)",
							...
							user.getInt("accountId"), isAdmin, isAdmin, roleId,
							currentTime, RegisterStatusConstants.ACTIVE,
							currentTime, RegisterSourceConstants.WEB);
		}

		jdbcTemplate
				.execute("ALTER TABLE `s_user` DROP COLUMN `registerStatus`");
		jdbcTemplate
				.execute("ALTER TABLE `s_user` DROP COLUMN `registrationSource`");

		jdbcTemplate
				.execute("ALTER TABLE `s_user` DROP FOREIGN KEY `s_user_fk_1` , DROP FOREIGN KEY `FK_s_user_3` ;ALTER TABLE `s_user` DROP COLUMN `roleid` , DROP COLUMN `isAdmin` , DROP COLUMN `accountId`, DROP INDEX `FK_s_user_3` , DROP INDEX `s_user_fk_1` ;");
