...
public class V1_26__Migrate_Comment_Attachment_Of_Project_Milestone implements
		SpringJdbcMigration {

	private static Logger log = LoggerFactory
			.getLogger(V1_26__Migrate_Comment_Attachment_Of_Project_Milestone.class);

	public void migrate(JdbcTemplate jdbcTemplate) throws Exception {
		try {

			List<Record> comments = jdbcTemplate
					.query("select m_comment.*, m_prj_milestone.projectid from m_comment,  m_prj_milestone WHERE m_comment.type=\"Project-Milestone\" AND m_prj_milestone.id=m_comment.typeid",
			...
			for (Record comment : comments) {
				...
				if (contents != null && contents.size() > 0) {
					...
					String messageNewPath = AttachmentUtils
							.getProjectMilestoneAttachmentPath(
