...
public class V1_33__Migrate_Attachment_Of_Project_Message implements
		SpringJdbcMigration {

	private static Logger log = LoggerFactory
			.getLogger(V1_33__Migrate_Attachment_Of_Project_Message.class);

	public void migrate(JdbcTemplate jdbcTemplate) throws Exception {
		try {

			List<Record> messages = jdbcTemplate.query(
					"select id, projectid, sAccountId from m_prj_message",
					new RowMapper<Record>() {
						...
						public Record mapRow(ResultSet rs, int rowNum)
							...
							message.put("sAccountId", rs.getInt("sAccountId"));
							message.put("id", rs.getInt("id"));
							message.put("projectId", rs.getInt("projectid"));
							return message;
						}
					});
			...
			for (Record message : messages) {
				String bugPath = AttachmentUtils.getMessagePath(
						message.getInt("sAccountId"), message.getInt("id"));

				List<Content> contents = resourceService.getContents(bugPath);
				if (contents != null && contents.size() > 0) {
					log.debug("Message {} has exist attachments, move them",
							message.getInt("id"));

					String taskAttachmentPath = AttachmentUtils
							.getProjectMessageAttachmentPath(
									message.getInt("sAccountId"),
									message.getInt("projectId"),
									message.getInt("id"));
