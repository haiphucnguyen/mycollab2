...
public class V1_30__Migrate_Comment_Attachment_Of_Project_Problem implements
		SpringJdbcMigration {

	private static Logger log = LoggerFactory
			.getLogger(V1_30__Migrate_Comment_Attachment_Of_Project_Problem.class);

	public void migrate(JdbcTemplate jdbcTemplate) throws Exception {
		try {

			List<Record> comments = jdbcTemplate
					.query("select m_comment.*, m_prj_problem.projectid from m_comment,  m_prj_problem WHERE m_comment.type=\"Project-Problem\" AND m_prj_problem.id=m_comment.typeid",
			...
			for (Record comment : comments) {
				...
				if (contents != null && contents.size() > 0) {
					...
					String messageNewPath = AttachmentUtils
							.getProjectProblemAttachmentPath(
