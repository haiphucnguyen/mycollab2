	...
	public class EditForm extends AdvancedEditBeanForm<ProjectRole> {
		...
		private class FormLayoutFactory extends ProjectRoleFormLayoutFactory {
			...
			private Layout createButtonControls() {
				return (new EditFormControlsGenerator<ProjectRole>(
						ProjectRoleAddViewImpl.EditForm.this))
			...
			protected Layout createBottomPanel() {
				...
				if (projectRole instanceof SimpleProjectRole) {
					perMap = ((SimpleProjectRole) projectRole)
							.getPermissionMap();
				} else {
					perMap = new PermissionMap();
				}

				GridFormLayoutHelper permissionFormHelper = new GridFormLayoutHelper(
						2,
		...
		public PermissionMap getPermissionMap() {
			...
			for (String permissionItem : permissionControlsMap.keySet()) {
				PermissionComboBox permissionBox = permissionControlsMap
						.get(permissionItem);
