...
public class ProjectInformationView extends VerticalLayout {
	...
	public ProjectInformationView() {
		...
		header.addComponent(projectName);

		TabSheet container = new TabSheet();
		// Project information
		container.addTab(constructProjectInformationView(),
				"Project Information");
	...
	private Component constructProjectInformationView() {
		...
		Label planStartDate = new Label(AppContext.formatDate(project
				.getPlanstartdate()));
		gridLayout.addComponent(planStartDate, "Plan Start Date", 0, 3);

		Label actualStartDate = new Label(AppContext.formatDate(project
				.getActualstartdate()));
		gridLayout.addComponent(actualStartDate, "Actual Start Date", 1, 3);

		Label planEndDate = new Label(AppContext.formatDate(project
				.getPlanenddate()));
		gridLayout.addComponent(planEndDate, "Plan End Date", 0, 4);

		Label actualEndDate = new Label(AppContext.formatDate(project
				.getActualenddate()));
