...
import com.esofthead.mycollab.module.user.accountsettings.profile.view.AdvancedInfoChangeWindow;
...
public class AccountSettingsViewImpl extends AbstractView implements
	...
	private void initUI() {
		...
		Label contentText = new Label("Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec a diam lectus. Sed sit amet ipsum mauris. Maecenas congue ligula ac quam viverra nec consectetur ante hendrerit...");
		...
		for(int i = 0; i < listSize; i++){
			...
			Label billingPrice = new Label("<span class='billing-price'>$" + plan.getPricing() + "</span>/month", Label.CONTENT_XHTML);
			billingPrice.addStyleName("billing-price-lbl");
			singlePlan.addComponent(billingPrice);
			
			Label billingUser = new Label("<span class='billing-user'>" + plan.getNumusers() + "</span>&nbsp;User", Label.CONTENT_XHTML);
			singlePlan.addComponent(billingUser);
			
			String planVolume = "";
			if(plan.getVolume() > 1024) {
				planVolume = String.format("%.2f", (((float) plan.getVolume()) /1024)) + "GB" ; 
			} else {
				planVolume =  plan.getVolume() + "MB";
			}
			
			Label billingStorage = new Label("<span class='billing-storage'>" + planVolume + "</span>&nbsp;Storage", Label.CONTENT_XHTML);
			singlePlan.addComponent(billingStorage);
			
			Label billingProject = new Label("<span class='billing-project'>" + plan.getNumprojects() +"</span>&nbsp;Project" + (plan.getNumprojects() > 1 ? "s" : ""), Label.CONTENT_XHTML);
			singlePlan.addComponent(billingProject);
			
			Label billingBugTracking;
			if(plan.getHasbugenable()) {
				billingBugTracking = new Label("Bugs Tracking", Label.CONTENT_DEFAULT);
			} else {
			...
			if(plan.getHasstandupmeetingenable()) {
				billingStandup = new Label("Standup Meeting", Label.CONTENT_DEFAULT);
			} else {
			...
			if(plan.getHastimetracking()) {
				billingTimeTracking = new Label("Time Tracking", Label.CONTENT_DEFAULT);
			} else {
			...
			Button selectThisPlan = new Button("Select", new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					AccountSettingsViewImpl.this.getWidget().getWindow().addWindow(new UpdateBillingPlanWindow(plan));					
				}
			});
			selectThisPlan.addStyleName(UIConstants.THEME_BLUE_LINK);
			singlePlan.addComponent(selectThisPlan);
			singlePlan.setComponentAlignment(selectThisPlan, Alignment.MIDDLE_CENTER);
	...
	private void loadCurrentPlan() {
		...
		Label currentBillingPrice = new Label("<span class='current-price'>$" + currentBillingPlan.getPricing() + "</span>/Month", Label.CONTENT_XHTML);
	...
	private class UpdateBillingPlanWindow extends Window {
		...
		private void initUI(VerticalLayout thisContent) {
			Label header = new Label("Are you sure you want to change your current billing plan to the following billing plan?");
			...
			Label chosenPlanPrice = new Label("<span class='billing-price'>$" + this.chosenPlan.getPricing() + "</span>/month", Label.CONTENT_XHTML);
			...
			thisContent.setComponentAlignment(chosenPlanInfo, Alignment.MIDDLE_CENTER);
			...
			thisContent.setComponentAlignment(controlBtns, Alignment.MIDDLE_RIGHT);
			thisContent.setSpacing(true);
			thisContent.setMargin(false, false, true,
					false);	
		}
