...
public class PagedTable extends Table {
	...
	public HorizontalLayout createControls() {
		...
		itemsPerPageSelect.addListener(new ValueChangeListener() {
			...
			public void valueChange(
				...
				itemsPerPage = Integer.valueOf(String.valueOf(event
		...
		addListener(new PageChangeListener() {
			...
			public void pageChanged(PagedTableChangeEvent event) {
				if (getCurrentPage() == getTotalAmountOfPages()) {
					adjustPageLength(container.getRealSize()
							- (itemsPerPage * (getTotalAmountOfPages() - 1)));

				} else {
					adjustPageLength(itemsPerPage);
	...
	public void setContainerDataSource(Container newDataSource) {
		...
		pagedTableContainer.setPageLength(itemsPerPage);
	...
	private void setPageFirstIndex(int firstIndex) {
		if (container != null) {
			...
			if (firstIndex > container.getRealSize() - 1) {
				...
				int pages = 0;
				if (itemsPerPage != 0) {
					pages = (int) Math.floor(0.0 + size / itemsPerPage);
				}
				firstIndex = pages * itemsPerPage;
			}
	...
	public void nextPage() {
		setPageFirstIndex(container.getStartIndex() + itemsPerPage);
	}
	...
	public void setCurrentPage(int page) {
		int newIndex = (page - 1) * itemsPerPage;
		if (newIndex < 0) {
			newIndex = 0;
		}
