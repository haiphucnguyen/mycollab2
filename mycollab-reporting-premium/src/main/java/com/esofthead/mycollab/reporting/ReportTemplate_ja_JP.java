 ...
 * @since 4.1.2
 */
class ReportTemplate_ja_JP extends AbstractReportTemplate {

    ReportTemplate_ja_JP() {
        ...
        groupStyle = stl.style(boldStyle).setHorizontalImageAlignment(HorizontalImageAlignment.LEFT);
    ...
    public ComponentBuilder<?, ?> createTitleComponent(String label) {
        ...
        ComponentBuilder<?, ?> dynamicReportsComponent = cmp.horizontalList(
                cmp.image(ReportTemplateFactory.class.getClassLoader().getResourceAsStream("images/logo.png"))
                        .setFixedDimension(150, 28), cmp.verticalList(cmp.text(label)
                                .setStyle(bold22CenteredStyle).setHorizontalTextAlignment(HorizontalTextAlignment.LEFT),
                        cmp.text("https://www.mycollab.com").setStyle(italicStyle).setHyperLink(link)));
