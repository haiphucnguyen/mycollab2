...
interface StandupReportMapperExt : ISearchableDAO<StandupReportSearchCriteria> {
    ...
    fun getProjectReportsStatistic(@Param("projectIds") projectIds: List<Int>, @Param("onDate") onDate: Date,
                                   rowBounds: RowBounds): List<StandupReportStatistic>

    fun findUsersNotDoReportYet(@Param("projectId") projectId: Int?, @Param("onDate") onDate: Date): List<SimpleUser>
}
