...
class SendingCountUserLoginByDateJob extends GenericQuartzJobBean {
  ...
  def executeJob(context: JobExecutionContext) {
    ...
    if (accessedUsers != null && accessedUsers.nonEmpty) {
      ...
      try {
        extMailService.sendHTMLMail(SiteConfiguration.getNoReplyEmail, SiteConfiguration.getNoReplyEmail, Arrays.asList(new MailRecipientField("hainguyen@esofthead.com", "Hai Nguyen")), null, null, contentGenerator.generateSubjectContent("Today system-logins count"), contentGenerator.generateBodyContent(COUNT_USER_LOGIN_TEMPLATE), null)
      }
      catch {
