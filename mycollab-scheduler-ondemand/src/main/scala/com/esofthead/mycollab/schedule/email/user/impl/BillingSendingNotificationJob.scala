...
import java.util.{Arrays, Calendar, Date, TimeZone}
...
import com.esofthead.mycollab.module.user.domain.{BillingAccount, BillingAccountWithOwners, BillingPlan}
...
class BillingSendingNotificationJob extends GenericQuartzJobBean {
  ...
  private val INFORM_EXPIRE_ACCOUNT_TEMPLATE: String = "templates/email/billing/informAccountIsExpiredNotification.mt"
  private val INFORM_FILLING_BILLING_INFORMATION_TEMPLATE: String = "templates/email/billing/remindAccountIsAboutExpiredNotification.mt"
...
//    }
  ...
  private def sendingEmailInformConvertToFreePlan(account: BillingAccountWithOwners) {
    ...
    for (user <- account.getOwners) {
      ...
      extMailService.sendHTMLMail(SiteConfiguration.getNoReplyEmail, SiteConfiguration.getSiteName, Arrays.asList(new MailRecipientField(user.getEmail, user.getDisplayName)), null, null, contentGenerator.generateSubjectContent("Your trial has been ended"), contentGenerator.generateBodyContent(INFORM_EXPIRE_ACCOUNT_TEMPLATE, SiteConfiguration.getDefaultLocale), null)
    }
  }
  ...
  private def sendRemindEmailAskUpdateBillingAccount(account: BillingAccountWithOwners, afterDay: Integer) {
    ...
    for (user <- account.getOwners) {
      ...
      contentGenerator.putVariable("userName", user.getUsername)
      contentGenerator.putVariable("link", link)
      extMailService.sendHTMLMail(SiteConfiguration.getNoReplyEmail, SiteConfiguration.getSiteName, Arrays.asList(new MailRecipientField(user.getEmail, user.getDisplayName)), null, null, contentGenerator.generateSubjectContent("Your trial is about to end"), contentGenerator.generateBodyContent(INFORM_FILLING_BILLING_INFORMATION_TEMPLATE, SiteConfiguration.getDefaultLocale), null)
    }
  }
}
