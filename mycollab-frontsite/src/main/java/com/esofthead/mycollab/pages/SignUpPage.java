...
public class SignUpPage extends BasePage {
	...
	private static final String GITHUB_LINK = "signupGitHub";
	...
	public SignUpPage(final PageParameters parameters) {
		...
		StatelessLink<Void> githubLink = new StatelessLink<Void>("signupGitHub") {
			private static final long serialVersionUID = 1L;

			@Override
			public void onClick() {
				doAuthenticate(GITHUB_LINK);
			}
		};
		githubLink.add(AttributeModifier.replace("class",
				new AbstractReadOnlyModel<String>() {
					private static final long serialVersionUID = 1L;
					...
					public String getObject() {
						if (currentService.equals("")
								|| currentService.equals(Constants.GITHUB)) {
							if (currentService.equals(Constants.GITHUB))
								return "service-selected";
							return "service-enabled";
						}
						return "service-disabled";
		...
		try {
			...
			if (GOOGLE_PLUS_LINK.equals(authId)) {
				...
				successUrl = SiteConfiguration.getSiteUrl()
						+ "oauth2/externalCallbackCommand";
				providerId = "linkedin";
			} else if (GITHUB_LINK.equals(authId)) {
				successUrl = SiteConfiguration.getSiteUrl()
						+ "oauth2/externalCallbackCommand";
				providerId = "github";
			} else {
	...
	public IModel getPageTitle() {
		return new Model<String>("Signup - Online Project Management, CRM, Document Management");
	}
