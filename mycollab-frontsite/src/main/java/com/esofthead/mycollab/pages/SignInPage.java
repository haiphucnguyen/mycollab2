...
import com.esofthead.mycollab.ext.SubdomainListPanel;
...
public class SignInPage extends BasePage {
	...
	public SignInPage(final PageParameters parameters){
		...
		final MarkupContainer listContainer = new WebMarkupContainer("listcontainer");
		...
		form.add(new AjaxButton("ajax-button", form) 
		{
			...
			public void onSubmit(AjaxRequestTarget target, Form<?> form) {
				...
				try {
					...
					final String[] response = userResource.getSubdomainsOfUser(emailString);
					
					if(response.length == 1) {
						String redirectUrl = "";
						if(Application.get().usesDevelopmentConfig()) {
							redirectUrl = ApplicationProperties.getProperty("webapp.url");
						} else {
						...
						for(String subdomainString : response) {
							final AbstractItem newItem = new AbstractItem(subdomainList.newChildId());
							
							Label subdomain = new Label("subdomain", subdomainString + ".mycollab.com");
							newItem.add(subdomain);
							
							ExternalLink gotosubdomainBtn = new ExternalLink("gotosubdomain", "https://"+subdomainString+".mycollab.com");
