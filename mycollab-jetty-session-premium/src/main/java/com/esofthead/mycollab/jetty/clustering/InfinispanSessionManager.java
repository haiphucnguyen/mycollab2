...
public class InfinispanSessionManager extends AbstractLifeCycle implements
		SessionManager {
	...
	private String sessionIdPathParameterNamePrefix = ";"
	...
	public HttpSession newHttpSession(HttpServletRequest request) {
		...
		InfinispanHttpSession session = new InfinispanHttpSession(id,
				maxIdleTime);
	...
	public HttpCookie getSessionCookie(HttpSession session, String contextPath,
		...
		String path = (cookieConfig.path == null) ? contextPath
				: cookieConfig.path;

		HttpCookie cookie = new HttpCookie(cookieConfig.name, session.getId(),
				cookieConfig.domain, (path == null || path.isEmpty()) ? "/"
						: path, cookieConfig.maxAge, cookieConfig.httpOnly,
	...
	public void complete(HttpSession httpSession) {
		...
		if (session.isValid() && session.isModified()) {
			...
			cache.replace(session.getId(), session, -1, TimeUnit.SECONDS,
					maxIdleTime, TimeUnit.SECONDS);
		}
	}
	...
	public void setSessionTrackingModes(Set<SessionTrackingMode> modes) {
		if (modes.size() != 1 || !modes.contains(SessionTrackingMode.COOKIE)) {
			throw new UnsupportedOperationException(
					"Only cookie sessions are supported");
		}
	}
	...
	public void cacheEntryCreated(
			CacheEntryCreatedEvent<String, InfinispanHttpSession> e) {
		...
		if (session != null) {
			...
			if (listeners != null) {
				...
				for (int i = 0; i < LazyList.size(listeners); i++) {
					((HttpSessionListener) LazyList.get(listeners, i))
	...
	public void cacheEntryRemoved(
			CacheEntryRemovedEvent<String, InfinispanHttpSession> e) {
	...
	@CacheEntriesEvicted
	public void cacheEntriesEvicted(
			CacheEntriesEvictedEvent<String, InfinispanHttpSession> e) {
		if (listeners != null) {
