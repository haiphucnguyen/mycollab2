...
public class InfinispanSessionIdManager extends AbstractSessionIdManager {
	 ...
	 * 
	 * @param cache
	 *            The cache to manage session IDs in.
	 */
	public InfinispanSessionIdManager(
			Cache<String, InfinispanHttpSession> cache, TimeUnit maxIdleUnit) {
		this.cache = cache;
	...
	public void addSession(HttpSession httpSession) {
		...
		cache.put(session.getId(), session, -1, maxIdleUnit,
				session.getMaxInactiveInterval(), maxIdleUnit);
	}
