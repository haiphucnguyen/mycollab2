...
public class LoginViewImpl extends AbstractMobileMainView implements LoginView {
    ...
    private void initUI() {
        ...
        Image mainLogo = new Image(null, AccountAssetsResolver.createLogoResource(AppUI.Companion.getBillingAccount().getLogopath(), 150));
        contentLayout.addComponent(mainLogo);

        CssLayout welcomeTextWrapper = new CssLayout();
        ELabel welcomeText = new ELabel(LocalizationHelper.getMessage(AppUI.Companion.getDefaultLocale(), ShellI18nEnum.BUTTON_LOG_IN))
                .withStyleName("h1");
        welcomeTextWrapper.addComponent(welcomeText);
        contentLayout.addComponent(welcomeText);

        final EmailField emailField = new EmailField();
        new Dom(emailField).setAttribute("placeholder", LocalizationHelper.getMessage(AppUI.Companion.getDefaultLocale(),
        ...
        new Dom(pwdField).setAttribute("placeholder", LocalizationHelper.getMessage(AppUI.Companion.getDefaultLocale(), ShellI18nEnum.FORM_PASSWORD));
        contentLayout.addComponent(pwdField);

        final CheckBox rememberPassword = new CheckBox(LocalizationHelper.getMessage(AppUI.Companion.getDefaultLocale(),
                ShellI18nEnum.OPT_REMEMBER_PASSWORD), true);
        rememberPassword.setWidth("100%");
        contentLayout.addComponent(rememberPassword);

        MButton signInBtn = new MButton(LocalizationHelper.getMessage(AppUI.Companion.getDefaultLocale(), ShellI18nEnum.BUTTON_LOG_IN), clickEvent -> {
            try {
