...
public class MilestoneAddViewImpl extends AbstractEditItemComp<SimpleMilestone>
	...
	protected String initFormTitle() {
		return (beanItem.getId() == null) ? AppContext
				.getMessage(MilestoneI18nEnum.FORM_NEW_TITLE) : beanItem
				.getName();
	}
		...
		public void attachField(Object propertyId, Field<?> field) {
			if (propertyId.equals("name")) {
				this.informationLayout.addComponent(field, AppContext
						.getMessage(MilestoneI18nEnum.FORM_NAME_FIELD), 0, 0);
			} else if (propertyId.equals("status")) {
				this.informationLayout.addComponent(field, AppContext
						.getMessage(MilestoneI18nEnum.FORM_STATUS_FIELD), 0, 1);
