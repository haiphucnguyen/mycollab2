...
class OpportunityRelayEmailNotificationActionImpl extends CrmDefaultSendingRelayEmailAction[SimpleOpportunity] with OpportunityRelayEmailNotificationAction {
  ...
  override protected def buildExtraTemplateVariables(context: MailContext[SimpleOpportunity]): Unit = {
    ...
    if (MonitorTypeConstants.CREATE_ACTION == emailNotification.getAction) {
      contentGenerator.putVariable("actionHeading", context.getMessage(OpportunityI18nEnum.MAIL_CREATE_ITEM_HEADING, makeChangeUser))
    }
    else if (MonitorTypeConstants.UPDATE_ACTION == emailNotification.getAction) {
      contentGenerator.putVariable("actionHeading", context.getMessage(OpportunityI18nEnum.MAIL_UPDATE_ITEM_HEADING, makeChangeUser))
    }
    else if (MonitorTypeConstants.ADD_COMMENT_ACTION == emailNotification.getAction) {
      contentGenerator.putVariable("actionHeading", context.getMessage(OpportunityI18nEnum.MAIL_COMMENT_ITEM_HEADING, makeChangeUser))
    }
    ...
    def formatField(context: MailContext[_]): String = {
      ...
      if (simpleOpportunity.getAccountid != null) {
        ...
        val accountLink: String = CrmLinkGenerator.generateAccountPreviewFullLink(context.siteUrl, simpleOpportunity
          .getAccountid)
        val link: A = TagBuilder.newA(accountLink, simpleOpportunity.getAccountName)
    ...
    def formatField(context: MailContext[_]): String = {
      ...
      if (opportunity.getCampaignid != null) {
        ...
        val campaignLink: String = CrmLinkGenerator.generateCampaignPreviewFullLink(context.siteUrl, opportunity
          .getCampaignid)
        val link: A = TagBuilder.newA(campaignLink, opportunity.getCampaignName)
        TagBuilder.newLink(img, link).write
      }
      else {
        new Span().write
  ...
  class AssigneeFieldFormat(fieldName: String, displayName: Enum[_]) extends FieldFormat(fieldName, displayName) {
    ...
    def formatField(context: MailContext[_]): String = {
      ...
      if (opportunity.getAssignuser != null) {
        ...
        val link: A = TagBuilder.newA(userLink, opportunity.getAssignUserFullName)
        TagBuilder.newLink(img, link).write
      }
      else {
        new Span().write
