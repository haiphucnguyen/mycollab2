...
class CampaignRelayEmailNotificationActionImpl extends CrmDefaultSendingRelayEmailAction[SimpleCampaign] with CampaignRelayEmailNotificationAction {
  ...
  override protected def buildExtraTemplateVariables(context: MailContext[SimpleCampaign]): Unit = {
    ...
    if (MonitorTypeConstants.CREATE_ACTION == emailNotification.getAction) {
      contentGenerator.putVariable("actionHeading", context.getMessage(CampaignI18nEnum.MAIL_CREATE_ITEM_HEADING, makeChangeUser))
    }
    else if (MonitorTypeConstants.UPDATE_ACTION == emailNotification.getAction) {
      contentGenerator.putVariable("actionHeading", context.getMessage(CampaignI18nEnum.MAIL_UPDATE_ITEM_HEADING, makeChangeUser))
    }
    else if (MonitorTypeConstants.ADD_COMMENT_ACTION == emailNotification.getAction) {
      contentGenerator.putVariable("actionHeading", context.getMessage(CampaignI18nEnum.MAIL_COMMENT_ITEM_HEADING, makeChangeUser))
    }
  ...
  class AssigneeFieldFormat(fieldName: String, displayName: Enum[_]) extends FieldFormat(fieldName, displayName) {
    ...
    def formatField(context: MailContext[_]): String = {
      ...
      if (campaign.getAssignuser != null) {
        ...
        val link: A = TagBuilder.newA(userLink, campaign.getAssignUserFullName)
